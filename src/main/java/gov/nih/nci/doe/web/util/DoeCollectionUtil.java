...
//    @Value("${gov.nih.nci.hpc.ssl.cert.password}")
	...
	public static boolean isCollectionEmpty(HpcCollectionDTO someCollection) {
		...
		if (null != someCollection && null != someCollection.getCollection()
				&& null != someCollection.getCollection().getSubCollections()
				&& null != someCollection.getCollection().getDataObjects()) {
			retVal = someCollection.getCollection().getSubCollections().isEmpty()
	...
	/**
	 * Retrieve from application properties the 3 property values needed to support
	 * checking whether current (authenticated) User has Ownership on a Collection.
	 *
	 * Provides the necessary property values as String array of length 3 where: -
	 * at index 0, element is URL for page displaying detailed info about a
	 * Collection - at index 1, element is location of keystore file - at index 2,
	 * element is password for keystore file
	 *
	 * For each of the 3 necessary properties, if it could not be found, then the
	 * returned String for its value is empty String.
	 *
	 * @return String array as described above
	 */
	private static final String[] fetchAppPropertiesForCollectionOwnershipCheck() {
		String[] propVals = new String[3];
		Arrays.fill(propVals, "");
		try {
			final Properties appProperties = new Properties();
			appProperties.load(DoeCollectionUtil.class.getResourceAsStream(CLASSPATH_LOC_4_APP_PROPS_FILE));
			final String serviceURL = fetchCollectionServiceURL(appProperties);
			final String sslCertPath = appProperties.getProperty(PROP_KEY__$SSL_CERT, "");
			final String sslCertPassword = appProperties.getProperty(PROP_KEY__$SSL_CERT_PASSWORD, "");
			propVals[0] = serviceURL;
			propVals[1] = sslCertPath;
			propVals[2] = sslCertPassword;
		} catch (IOException ioe) {
			throw new RuntimeException(String.format("%s file was not found on classpath!", APP_PROPERTIES_FILE));
		}
		return propVals;
	}
	...
	private static String fetchCollectionServiceURL(Properties appProps) {
		...
		final String rawServiceURL = appProps.getProperty(PROP_KEY__$URI_COLLECTION);
		if (null != rawServiceURL) {
			final String prefix2Replace = "${".concat(PROP_KEY__$REST_API_ROOT_URL).concat("}");
			final String rootURL = appProps.getProperty(PROP_KEY__$REST_API_ROOT_URL);
